...
Benchmark.ips do |x|
  ...
  x.report('B_direct') do
    Smullyan::Birds::B_direct.(double).(add_one).(5)
  end
...
Benchmark.ips do |x|
  ...
  x.report('C (derived)') do
    Smullyan::Birds::C.(subtract).(10).(3)
  end

  x.report('C_direct') do
    Smullyan::Birds::C_direct.(subtract).(10).(3)
  end
...
Benchmark.ips do |x|
  ...
  x.report('W (derived)') do
    Smullyan::Birds::W.(pair).(21)
  end

  x.report('W_direct') do
    Smullyan::Birds::W_direct.(pair).(21)
  end
...
Benchmark.ips do |x|
  x.report('M (derived)') do
    Smullyan::Birds::M.(const_forty_two)
  end

  x.report('M_direct') do
    Smullyan::Birds::M_direct.(const_forty_two)
  end
...
Benchmark.ips do |x|
  x.report('L (derived)') do
    Smullyan::Birds::L.(identity).(const_forty_two)
  end

  x.report('L_direct') do
    Smullyan::Birds::L_direct.(identity).(const_forty_two)
  end
...
Benchmark.ips do |x|
  ...
  sum_to_n = ->(f) { ->(n) { n <= 0 ? 0 : n + f.(n - 1) } }

  x.report('Y combinator') do
    Smullyan::Birds::Y.(sum_to_n).(10)
  end

  x.report('Z combinator') do
    Smullyan::Birds::Z.(sum_to_n).(10)
  end
